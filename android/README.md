...
这个 Android 客户端使用独立包名 `app.atherloom.react`，不会覆盖旧版 Atherloom。APK 内置 React/Vite 构建产物，默认不需要电脑后端：API Key 保存在 Android 加密存储中，设置、人格、世界书、会话和消息保存在本机，模型请求通过原生网络桥直连。

如需与现有 FastAPI/SQLite 共用数据，可在“设置 → 后端连接”填写后端根地址，例如：

```text
http://192.168.1.20:8876
```

清空后端地址并保存会切回 Android 本机模式。连接服务器时，手机与后端必须能够互相访问；局域网 FastAPI 需要监听 `0.0.0.0`，Windows 防火墙也需要允许相应端口，公网应配置 HTTPS。
