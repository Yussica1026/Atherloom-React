...
> 本文描述已并入 v0.2.3 发布候选的语音实现。固定签名 APK 尚待标签构建核验；真实 Android 手机上的麦克风、系统识别和音频播放验收仍未完成。
...
- production build 已通过；完成 Android Gradle 构建、固定签名与公开附件核验后再把 v0.2.3 APK 标为可分发，最终证据记录在工作日志中。
